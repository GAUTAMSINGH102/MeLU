...
class user_preference_estimator(torch.nn.Module):
    ...
    def forward(self, x, training = True):
        ...
        tag_idx = Variable(x[:, 0:82], requires_grad=False)
        type_idx = Variable(x[:, 82], requires_grad=False)
        city_idx = Variable(x[:, 83], requires_grad=False)
        area_idx = Variable(x[:, 84], requires_grad=False)
        ...
        item_emb = self.item_emb(tag_idx, type_idx)
        # print(item_emb)
        user_emb = self.user_emb(city_idx, area_idx)
        # print(user_emb)
